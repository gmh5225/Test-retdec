...
struct MCInst {
    ...
    uint8_t imm_offset[2]; // immediate offset, some instruction may have 2 immediates
    uint8_t imm_read_size[2]; // size of bytes read of immediate operand.
    uint8_t displace_offset;       // displacement offset
    uint8_t displacement_raw_size; // raw size of displacement, in bytes
