...
typedef struct x86_op_mem {
    ...
    uint8_t disp_off;      // offset from the start of the instruction to the pieces of data.
    uint8_t disp_raw_size; // size of displacement, in bytes
} x86_op_mem;

// Instruction's operand of immediate
// This is associated with X86_OP_IMM operand type above
typedef struct x86_op_imm
{
    int64_t imm;
    uint8_t offset;        // offset from the start of the instruction to hte pieces of data.
    uint8_t raw_byte_size; // raw size in instruction, in bytes
} x86_op_imm;
...
typedef struct cs_x86_op {
		...
		union {
            ...
            x86_op_imm imm; // immediate value for IMM operand
