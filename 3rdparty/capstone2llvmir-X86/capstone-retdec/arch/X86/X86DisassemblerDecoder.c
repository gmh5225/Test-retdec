...
static int readDisplacement(struct InternalInstruction *insn)
{
	...
	switch (insn->eaDisplacement) {
		case EA_DISP_NONE:
            ...
            insn->displacement_raw_size = 0;
			break;
		case EA_DISP_8:
			if (consumeInt8(insn, &d8))
				return -1;
			insn->displacement = d8;
            insn->displacement_raw_size = 1;
			break;
		case EA_DISP_16:
			if (consumeInt16(insn, &d16))
				return -1;
			insn->displacement = d16;
            insn->displacement_raw_size = 2;
			break;
		case EA_DISP_32:
			if (consumeInt32(insn, &d32))
				return -1;
			insn->displacement = d32;
            insn->displacement_raw_size = 4;
			break;
	}
...
static int readImmediate(struct InternalInstruction *insn, uint8_t size)
{
    ...
    insn->immediateOffset[insn->numImmediatesConsumed] = (uint8_t)(insn->readerCursor - insn->startLocation);
	...
	switch (size) {
		case 1:
            ...
            insn->immediateSizeRead[insn->numImmediatesConsumed] = 1;
			break;
		case 2:
			if (consumeUInt16(insn, &imm16))
				return -1;
			insn->immediates[insn->numImmediatesConsumed] = imm16;
            insn->immediateSizeRead[insn->numImmediatesConsumed] = 2;
			break;
		case 4:
			if (consumeUInt32(insn, &imm32))
				return -1;
			insn->immediates[insn->numImmediatesConsumed] = imm32;
            insn->immediateSizeRead[insn->numImmediatesConsumed] = 4;
			break;
		case 8:
			if (consumeUInt64(insn, &imm64))
				return -1;
			insn->immediates[insn->numImmediatesConsumed] = imm64;
            insn->immediateSizeRead[insn->numImmediatesConsumed] = 8;
			break;
	}
