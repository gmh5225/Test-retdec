	...
	if (ret) {
		...
		*size = (uint16_t)insn.length;
		...
		if (result) {
            ...
            {
                instr->displace_offset = insn.displacementOffset;
                instr->displacement_raw_size = insn.displacement_raw_size;
            }
            while (insn.numImmediatesConsumed--)
            {
                instr->imm_offset[insn.numImmediatesConsumed] = insn.immediateOffset[insn.numImmediatesConsumed];
                instr->imm_read_size[insn.numImmediatesConsumed] = insn.immediateSizeRead[insn.numImmediatesConsumed];
            }
