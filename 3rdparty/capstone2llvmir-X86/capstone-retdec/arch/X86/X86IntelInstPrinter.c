...
static void printU8Imm(MCInst *MI, unsigned Op, SStream *O)
{
	...
	if (MI->csh->detail) {
		...
		MI->flat_insn->detail->x86.operands[MI->flat_insn->detail->x86.op_count].imm.imm = val;
...
void X86_Intel_printInst(MCInst *MI, SStream *O, void *Info)
{
	...
	if (MI->csh->detail) {
		...
		for (n = 0, j = 0; n != MI->flat_insn->detail->x86.op_count; n++)
        {
            op = &MI->flat_insn->detail->x86.operands[n];
            if (op->type == X86_OP_IMM)
            {
                op->imm.offset = MI->imm_offset[j];
                op->imm.raw_byte_size = MI->imm_read_size[j++];
            }
            else if (op->type == X86_OP_MEM)
            {
                op->mem.disp_off = MI->displace_offset;
                op->mem.disp_raw_size = MI->displacement_raw_size;
            }
        }
...
static void printPCRelImm(MCInst *MI, unsigned OpNo, SStream *O)
{
	...
	if (MCOperand_isImm(Op)) {
		...
		if (MI->csh->detail) {
			...
			MI->flat_insn->detail->x86.operands[MI->flat_insn->detail->x86.op_count].imm.imm = imm;
